...
fname = 'jazzmen_narrow.txt'
nodes = []
links = []

with open(fname) as f:
    jazzmen = f.readlines()
jazzmen = [(x.strip('\n').split("  ")[0],x.strip('\n').split("  ")[1]) for x in jazzmen]
#print(jazzmen)
for i in jazzmen:
    node = {"name": i[0], "group": i[1], "max": 0}
    nodes.append(node)
print(nodes)
...
for i in links:
    ...
    if(i["value"] > nodes[i["source"]]["max"]):
        nodes[i["source"]]["max"] = i["value"]
    if(i["value"] > nodes[i["target"]]["max"]):
        nodes[i["target"]]["max"] = i["value"]
